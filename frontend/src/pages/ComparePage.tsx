...
import { ArrowDownIcon, ArrowRightIcon, BookOpenIcon, BrainIcon, CheckCircleIcon, LightningIcon, PlayIcon, RepeatIcon, SparkleIcon } from "@phosphor-icons/react";
import { getComparison, getBook } from "../api";
import { BookCover, BookDetail, LoadingRoute } from "../components";
import { useApp } from "../state";
import type { Book, CompareResponse } from "../types";
...
export default function ComparePage() {
  ...
  async function openBook(book: Book) { try { setSelectedBook(await getBook(book.id) || book); } catch { console.error("无法读取书籍详情"); } }
  ...
  return <div className="compare-page page-enter">
    ...
    <div className="comparison-board">
      <section><div className="comparison-title"><span>反馈前</span><h3>只依据显式条件</h3><small>引用记忆 0 条</small></div><div className="comparison-books">{data.first_recommendation.books.slice(0, 4).map((score, index) => <div key={score.book.id} className="comparison-book-item" onClick={() => openBook(score.book)}><span className="compare-rank">#{index + 1}</span><BookCover book={score.book} size="sm" /><span><strong>{score.book.title}</strong><small>{score.book.pages} 页 · 案例 {Math.round(score.book.case_ratio * 100)}%</small></span><code>{score.total_score.toFixed(1)}</code><BookOpenIcon className="book-open-icon" /></div>)}</div></section>
      <div className="change-spine"><span><LightningIcon /></span><strong>记忆影响</strong><i /><p>短篇实战书上升</p><p>长篇理论书下降</p><p>偏好评分最高 +30%</p></div>
      <section className={step >= 3 ? "revealed" : "obscured"}><div className="comparison-title after"><span>记忆后</span><h3>依据反馈重新排序</h3><small>引用记忆 {data.second_recommendation.memories_used} 条</small></div><div className="comparison-books">{data.second_recommendation.books.slice(0, 4).map((score, index) => { const before = beforeMap.get(score.book.id) || index + 1; const change = before - (index + 1); return <div key={score.book.id} className="comparison-book-item" onClick={() => openBook(score.book)}><span className="compare-rank">#{index + 1}</span><BookCover book={score.book} size="sm" /><span><strong>{score.book.title}</strong><small>{score.book.pages} 页 · 案例 {Math.round(score.book.case_ratio * 100)}%</small></span><code>{score.total_score.toFixed(1)}</code><em className={change > 0 ? "up" : change < 0 ? "down" : "same"}>{change > 0 ? `↑ ${change}` : change < 0 ? `↓ ${Math.abs(change)}` : "-"}</em><BookOpenIcon className="book-open-icon" /></div>; })}</div></section>
    </div>
