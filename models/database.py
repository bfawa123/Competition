...
class Database:
    ...
    def search_books(self, filters: dict) -> List[Book]:
        ...
        if filters.get("difficulty"):
            books = [b for b in books if b.difficulty == filters["difficulty"]]
    ...
    def save_memory(self, memory: Memory):
        ...
        # 检查是否已存在相同 field 的记忆，更新或新增
        existing_idx = None
        for idx, mem in enumerate(all_memories[user_id]):
            if mem["field"] == memory.field:
                existing_idx = idx
                break
